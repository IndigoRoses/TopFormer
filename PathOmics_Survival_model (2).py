...
#             slide_sub_labels.append(tslideLabel)
            ...
            tAA = self.attention(tmidFeat).squeeze(0)  # shape (n)
            tmidFeat = tmidFeat.squeeze(0) if tmidFeat.dim() == 3 else tmidFeat  # ensure (n, d)
            tattFeats = torch.einsum('ns,n->ns', tmidFeat, tAA)
            # Average to get one vector per sub-group
            af_inst_feat = tattFeats.mean(dim=0, keepdim=True)  # 1 x 256
